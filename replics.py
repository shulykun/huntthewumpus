class Replics():
    ...
    def game_over(self):
        r = '''##-- ИГРА ОКОНЧЕНА --# Наберите «старт» или «выход».'''
        return r
    ...
    def shoot_wrong_room(self, room_numbers, room_target, room_prev, arrows_count):
        ...
        if arrows_count == 0:
            response_arr_count = '''У вас не осталось стрел. Это поражение :-(
            ##-- ИГРА ОКОНЧЕНА --# Наберите «старт» или «выход».'''
        else:
            response_arr_count = 'Осталось стрел: {}'.format(arrows_count)
    ...
    def shoot_wampus_killed(self,room_numbers,room_target):
        ...
        a = "Выстрел! Стрела нацелена в комнаты {}.".format(room_numbers)
        b = '''Это победа! Стрела поразила Вампуса в комнате {}. Вы нашли выход из лабиринта.
        ##-- ВЫ ПОБЕДИЛИ --#  Наберите «старт» или «выход».'''.format(room_target)
    ...
    def shoot_player_killed(self,room_numbers,room_target):
        ...
        ##-- ИГРА ОКОНЧЕНА --# Наберите «старт» или «выход».'''.format(room_target)
    ...
    def not_found(self):
        ...
        return '''Я не могу понять этот запрос.
        # Попробуйте набрать: «старт», «идти», «комната», «стрелять», «где я» «помощь», «тролль».'''
    ...
    def help(self):
        r = '''В игре доступны команды:#«Идти» или «Комната» - надо указать номер смежной комнаты.#
                «Стрелять» - надо указать номера 5 комнат, через которые пролетит стрела.#
                «Состояние» - чтобы узнать сколько у вас стрел.
                «Тролль» - вызов тролля чтобы поиграть и получить подсказку.
                «Где я» - напоминание где вы находитесь.'''
        return r
